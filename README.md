...
All four games can create mixed human/AI rooms. 斗地主 and 扎金花 use fixed three-seat tables; Texas Hold’em supports 3–6 seats; Blackjack supports 1–5 seats. The host can invite humans, fill every empty seat with server-side strategy AI, or remove bots before starting. The Site creates and joins rooms, while `codex-poker-rooms` runs one Cloudflare Durable Object per room with hibernating WebSockets, durable state, 45-second turn alarms, seat-preserving reconnects, private hands and server-side result settlement. Run the local pair with `npm run dev` and `npx wrangler dev --config rooms/wrangler.jsonc --port 8787`, then execute `npm run test:room:e2e`.
